...
const ProductCard = ({ product, onAddToBasket }) => {
    ...
    const karatLabel = p.type || p.karat || "21";
    const weightLabel = p.weight ? `${p.weight} جرام` : "غير محدد";
        ...
        <div className='w-full max-w-[320px] sm:w-[320px] shrink-0 mx-auto h-85 aspect-4/4 order-1 md:order-2 bg-card rounded-xl border border-white/10 shadow-2xl relative overflow-hidden hover:border-primary/50 transition duration-300 group hover:-translate-y-2'>
            ...
            <div className='product-info flex flex-col'>
                ...
                <span className='flex flex-row justify-around lg:justify-center gap-10 lg:gap-18 pb-2.5'>
                    <span className='flex flex-row-reverse gap-2 flex-nowrap'><p className="text-xs md:text-base lg:text-sm">عيار {karatLabel}</p> <img src={sun} alt="" className='w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-5 lg:h-5 xl:w-6 xl:h-6' /></span>
                    <span className='flex flex-row-reverse gap-2'><p className="text-xs md:text-base lg:text-sm">{weightLabel}</p> <img src={weightIcon} alt="" className='w-4 h-4 sm:w-4 sm:h-4 md:w-5 md:h-5 lg:w-6 lg:h-6' /></span>
                </span>
                <div className="flex flex-row justify-between items-center border-t border-white/10 pt-4 m-5">
                    <div className="flex flex-col">
                        ...
                        <p className='text-primary font-bold text-sm md:text-base lg:text-lg'>{priceValue} دينار</p>
                    </div>
                    {onAddToBasket && (
                        <button
